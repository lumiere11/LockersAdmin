...
const PanelInfoLockers = ({
  earnings,
  totalLockersEarnings,
  lockerSelect,
}: IPanelInfoLockers) => {
  return (
    <PanelInfoLockersWrapper>
      <Card>
        <Card.Body className={`${styles["card-body-info"]}`}>
          <Row className={`${earnings.length > 1 ? '' : 'd-flex justify-content-center' } `}>
            <Col xs="12" md="8">
              <Row >
                {earnings.map((item, index) => (
                  <>
                    <Col
                      xs="12"
                      md="6"
                      className={`mb-4 cursor-pointer ${styles.locker}`}
                      onClick={() => lockerSelect(false, item.lockerId)}
                    >
                      <Card>
